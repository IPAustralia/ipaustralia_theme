...
/* mwheelIntent.js */
(function(a){function g(){if(this===c.elem){c.pos=[-260,-260];c.elem=false;d=3}}var c={pos:[-260,-260]},d=3,e=document,k=e.documentElement,h=e.body,i,j;a.event.special.mwheelIntent={setup:function(){var b=a(this).bind("mousewheel",a.event.special.mwheelIntent.handler);this!==e&&this!==k&&this!==h&&b.bind("mouseleave",g);return true},teardown:function(){a(this).unbind("mousewheel",a.event.special.mwheelIntent.handler).unbind("mouseleave",g);return true},handler:function(b){var f=[b.clientX,b.clientY];
if(this===c.elem||Math.abs(c.pos[0]-f[0])>d||Math.abs(c.pos[1]-f[1])>d){c.elem=this;c.pos=f;d=250;clearTimeout(j);j=setTimeout(function(){d=10},200);clearTimeout(i);i=setTimeout(function(){d=3},1500);b=a.extend({},b,{type:"mwheelIntent"});return a.event.handle.apply(this,arguments)}}};a.fn.extend({mwheelIntent:function(b){return b?this.bind("mwheelIntent",b):this.trigger("mwheelIntent")},unmwheelIntent:function(b){return this.unbind("mwheelIntent",b)}});a(function(){h=e.body;a(e).bind("mwheelIntent.mwheelIntentDefault",
a.noop)})})(jQuery);
...
/* wizard.js */
var ND=function(s,b){var w={step1:'<div class="row"><input type="radio" name="step%STEP%" id="step%STEP%task%ID%" value="step%STEP%task%ID%" /><label for="step%STEP%task%ID%">%LABEL%%HELP%</label></div>',step2group:'<div class="options" id="option-step1task%ID%"><h2>%LABEL%</h2></div>',step2:'<div class="row"><input type="radio" name="step%STEP%" id="step%STEP%task%ID%" value="%VALUE%" /><label for="step%STEP%task%ID%">%LABEL%%HELP%</label></div>',step3:'<div class="annotations" id="annotation-%ID%">%CONTENT%</div>'},
m=function(k,l){return b(w[k].replace(/%([A-Z]*?)%/g,function(t,i){return l[i.toLowerCase()]}))};s.wizard=function(){function k(a){b("<img />").attr("src",a.imgSupport+"?_="+ +new Date).load(function(){b("html").addClass("img-support")});if(window.location.href.indexOf("text-only=true")!==-1||!e||!e.size()||!a.contents||!a.data||e.hasClass("ie6")){e.removeClass("not-loaded");return false}u=e.find(".annotations");e.removeClass("disabled").removeClass("not-loaded").empty().html('<p class="loading">Loading content...</p>');
return true}function l(a,f){n=f.data;b.extend(b.ui.accordion.animations,{slowslide:function(d){b.ui.accordion.animations.slide(d,{duration:600})}});e.html(a).accordion({autoHeight:true,animated:"slowslide"});t();x()}function t(){var a;o=b("#step1-contents");b.each(n.conf,function(f,d){d=m("step1",b.extend(d,{step:1,help:i(d)}));if(f===0)a=d;o.append(d)});y();o.find("INPUT").change(z);a.find("INPUT").attr("checked","checked").trigger("change")}function i(){return""}function z(){p=false;var a=q.hide().filter("#option-"+
this.value).show().find("input");a.eq(0).attr("checked","checked").trigger("change");if(a.size()<=1){a.eq(0).trigger("change");p=true}}function y(){var a;step2contents=b("#step2-contents");b.each(n.conf,function(f,d){var g=m("step2group",d);step2contents.append(g);b.each(d.children,function(h,c){c=m("step2",b.extend(c,{id:c.id+"-"+f+"-"+h,value:c.id,step:2,help:i(c)}));if(f===0&&h===0)a=c;g.append(c)})});q=step2contents.find(".options");A();q.find("INPUT").change(B);a.find("INPUT").attr("checked",
"checked").trigger("change")}function B(){var a=v.hide().filter(".annotation-"+this.value).show().find(".for-more");r.hide();a.size()&&r.show(0).attr("href",a.eq(0).attr("href"))}function C(a,f){f.newContent.is(".step3")&&b("html").hasClass("img-support")&&j.jScrollPane({contentWidth:385,showArrows:true})}function A(){r=b("#moreLink");j=b("#step3-contents");j.append(u);v=j.find(".annotations")}function x(){function a(c){if(c.hasClass("open")){h=false;c.removeClass("open")}}function f(c){if(!c.hasClass("open")){h=
c;c.addClass("open")}}function d(c){c.preventDefault();f(b(this))}function g(c){c&&c.preventDefault();a(b(this))}e.find("button.next, button.back").click(function(){var c=b(this).val().split("-");e.accordion("activate",parseInt(c[1]==1&&p?c[0]==="f"?2:0:c[1]))});e.bind("accordionchange",C);var h;e.click(function(){h&&g.apply(h,[])});e.delegate(".help","click",d).delegate(".help","focusin",d).delegate(".help","focusout",g).delegate(".help","blur",g)}var e,n,r,o,q,j,v,u,p=false;return{init:function(a){e=
b("#wizard");if(k(a)){var f="?_="+ +new Date;b.when(b.get(a.contents+f),b.getJSON(a.data+f)).done(function(d,g){l(d[0],g[0])});return this}}}};return s}(window.ND||{},jQuery);
/* global.js */
(function(a,b){b(function(){a.wizard().init({contents:"images/contents.html",data:"images/data.js",imgSupport:"images/test.png"})})})(window.ND||{},jQuery.noConflict());
